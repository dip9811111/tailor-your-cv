...
st.markdown(
    "<h1 style='text-align: center;'>AI CV Builder</h1>", unsafe_allow_html=True
)
...
with st.sidebar.expander("🔐 Models & Info", expanded=True):
    gemini_api_key = st.text_input(
        "Enter your Gemini API key", value=gemini_api_key_value, type="password"
    )
    openai_api_key = st.text_input(
        "Enter your OpenAI API key", value=openai_api_key_value, type="password"
    )
    uploaded_file = st.file_uploader(
        "Upload your CV file", type=["pdf", "txt", "docx", "md"]
    )
...
with left_col:
            ...
            if gemini_api_key or openai_api_key:
                with st.spinner("Processing file..."):
                    ...
                    if markdown_cv:
                        ...
                        if gemini_api_key:
                            st.session_state.information_extractor.MODEL = (
                                load_gemini_model()
                            )
                        else:
                            st.session_state.information_extractor.MODEL = (
                ...
                if not error_:
                    ...
                    st.warning(
                        "Error processing file. Are you sure file extension is one of: .pdf, .txt, .docx, .md?"
                    )
            else:
    ...
    if st.session_state.get("generate_cv_trigger"):
        ...
        if (
            not (gemini_api_key or openai_api_key)
            or not os.path.exists(f"{dest_dir}/user_curriculum.md")
            or not job_description
        ):
            st.warning(
                "Please provide an API key, upload a CV file and paste a job description."
            )
        else:
            ...
            with st.spinner("Generating your CV..."):
                try:
                    ...
                    st.session_state.final_cv_content = (
                        st.session_state.information_extractor.final_cv
                    )
...
with right_col:
    if "final_cv_content" in st.session_state and "generated_html" in st.session_state:
        html(
            a4_style.format(st.session_state.generated_html),
            height=1300,
            scrolling=True,
        )
